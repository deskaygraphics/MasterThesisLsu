...
%% Center the Table of Contents, List of Tables, and List of Figures titles
\makeatletter
...
\degrees{M.S., Louisiana State University, 2026}
...
Land subsidence refers to the vertical lowering of the ground surface, occurring either gradually over extended periods or abruptly in response to triggering events. Although natural phenomena including sediment compaction, post-glacial rebound, and plate tectonics play a role in surface deformation, anthropogenic influences have emerged as the primary cause of severe subsidence events globally, with human activities contributing to over 80\% of high-impact cases documented worldwide \citep{Higgins2016}. Intensive groundwater pumping, compression of aquifer systems, consolidation of compressible soils, and structural loading from urban development collectively accelerate subsidence rates, threatening infrastructure integrity, water resource sustainability, and flood protection systems in expanding metropolitan areas.
...
Conventional approaches to subsidence monitoring, such as geotechnical investigations, geodetic levelling, and GNSS/GPS surveys, deliver centimeter-level accuracy but only at isolated measurement points. These methods incur substantial costs and lack the spatial density required for comprehensive regional hazard assessment. Satellite-based InSAR has fundamentally changed the monitoring paradigm by providing spatially continuous displacement measurements at millimeter precision across expansive regions. Multi-temporal InSAR methods, particularly PS-InSAR and SBAS-InSAR, have demonstrated effectiveness in capturing gradual deformation trends across diverse landscapes including urbanized and vegetated terrain \citep{Ferretti2001, Berardino2002, Ferretti2011}.

Translating extensive InSAR datasets into dependable spatio-temporal predictions remains difficult given the inherently non-linear and multi-factorial character of subsidence phenomena. Numerical geomechanical models provide mechanistic understanding but demand comprehensive subsurface parameterization that is seldom available at regional extents \citep{Zhou2022}. Machine learning approaches have emerged as promising alternatives, capable of extracting complex patterns linking deformation to environmental predictors directly from observational data. Among these, the Extra Trees algorithm has exhibited superior performance in susceptibility mapping applications, attributed to its resilience against predictor correlation, capacity to represent non-linear feature interactions, and reduced tendency toward overfitting relative to standard Random Forest implementations \citep{HosseinzadehLLand2024}.
...
InSAR-derived deformation was validated using GNSS vertical displacement time series
 from the GulfNet CORS stations (1LSU, GVMS, and SJB1) in EBRP obtained from the Nevada Geodetic
...
Within this expression, $\phi(t_A, x, r)$ and $\phi(t_B, x, r)$ represent phase measurements at location $(x, r)$ for acquisition times $t_A$ and $t_B$. The component $\Delta\phi_{topo}(x,r)$ captures residual terrain-related phase in the differential interferogram, $\Delta\phi_{atm}(x,r)$ encompasses atmospheric propagation delay contributions, and $\Delta\phi_{noise}(x,r)$ accounts for decorrelation noise. Given available DEM data, topographic phase can be modelled and subtracted to eliminate elevation-induced phase artifacts \citep{Fattahi2015}. Assembling the phase observations from all differential interferograms into matrix $\delta\phi$ and the mean deformation velocities into matrix $V$, the system relationship becomes:
\begin{equation}
BV = \delta\phi
\end{equation}
...
The coefficient of determination ($R^2$) measures the fraction of observed subsidence velocity variance accounted for by model predictions \citep{Kutner2005}. The computation follows:
\begin{equation}
...
SBAS-InSAR analysis of 246 Sentinel-1 acquisitions (2017--2025) revealed spatially heterogeneous deformation with mean velocity of $+0.46 \pm 3.98$~mm/yr. Maximum subsidence ($-23.66$~mm/yr) occurred near fault systems and groundwater extraction zones, while uplift ($+19.18$~mm/yr) was observed in northern forested regions. Validation against three GNSS stations confirmed excellent agreement with a mean velocity difference of only 0.15~mm/yr.

Extra Trees regression achieved superior performance ($R^2 = 0.9236$, RMSE = 1.10~mm/yr) compared to Random Forest ($R^2 = 0.8825$). Explainability analysis using MDI, SHAP, and LIME consistently identified land use/land cover, fault proximity, elevation, and groundwater well influence as the primary subsidence drivers.
