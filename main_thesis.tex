...
MDI-based feature importance rankings (Figure~\ref{fig:mdi_importance}) identify the top five predictors: (1) LULC (23.4\% relative importance), (2) DTF (18.7\%), (3) DTRDEM interaction (12.3\%), (4) DEM (9.8\%), and (5) Well (8.2\%). The dominance of LULC reflects the critical role of anthropogenic land modification in controlling subsidence susceptibility, consistent with findings from other urbanizing regions \citep{yu_explainable_2025}. The prominence of interaction terms (DTRDEM, SlopeDEM) demonstrates that subsidence is controlled by coupled processes rather than individual factors acting independently.
...
\end{tabular}
\caption{SHAP summary plots: (a) Extra Trees; (b) Random Forest. Color scale in (b): red = high feature value, blue = low.}
