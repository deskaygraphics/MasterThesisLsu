...
The use of POEORB significantly improves satellite position accuracy compared to restituted Orbits and reduces long
wavelength phase ramps in Interferometric products. This step is essential for reliable deformation timeseries analysis,
particularly for long temporal stacks \citep{ESA,2021}.
\subsection{Geospatial Conditioning Factors}
Twenty (20) geospatial conditioning factors were prepared at 30 m resolution and 
aligned to a common grid. Key groups include:
\begin{itemize}
\item Topographic: elevation (30 m SRTM), slope, aspect, curvature, Topographic 
  Wetness Index (TWI).
\item Geological/hydrogeological: soil type, lithology, distance to Baton Rouge 
  and Denham Springs fault lines.
\item Anthropogenic/environmental: Land Use/Land Cover (LULC), NDVI, distances to 
  roads, rivers, groundwater wells, railways, and bridges.
\end{itemize}
Continuous layers were min--max scaled; categorical layers were one-hot encoded.

\subsection{Validation Data}
GNSS vertical displacement time series from the GulfNet CORS network (notably 
stations 1LSU and GMVS within EBRP) are used for independent validation of 
